...
export default defineConfig({
  ...
  vite: {
    resolve: {
      alias: {
        // PDF resume extraction on Cloudflare Workers (src/lib/premium/resume-extract.server.ts).
        // pdf-parse's documented serverless entry, `pdf-parse/worker`, statically imports the
        // NATIVE addon `@napi-rs/canvas` to install DOMMatrix/Path2D/ImageData. A Worker cannot
        // load `.node` binaries, so the build fails (UNLOADABLE_DEPENDENCY) and, without
        // DOMMatrix, pdf.js throws while loading. We only ever call getText(), which never
        // renders, so the addon is replaced by a tiny text-only shim in this build. Vitest has
        // its own config and keeps using the real package under Node.
        "@napi-rs/canvas": fileURLToPath(
          new URL("./src/lib/premium/napi-canvas-shim.ts", import.meta.url),
        ),
      },
    },
    build: {
